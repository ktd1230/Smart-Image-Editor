...
def mystory(request):
    if request.method == 'POST':
        ...
        for i in range(len(image)):
            ...
            if i == 0:
                firstimage = image[i]
        story = Story.objects.create(title=title, image=img, text=text, firstimage=firstimage, user=request.user)
        story.save()
        ...
        return JsonResponse({"result":"true","data":serializers.data,"first_image":first_image})
...
def like(request, id):
    ...
    if user not in story.like_users.all():
        ...
        on_like = False
    return JsonResponse({"result":"true","count_like":story.like_users.all().count(),"on_like":on_like})
